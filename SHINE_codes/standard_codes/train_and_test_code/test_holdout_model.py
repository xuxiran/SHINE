...
def load_model_and_checkpoint(checkpoint_file):
    # Construct the module name and class name
    version = checkpoint_file.split('_')[2]  # Extract version from the file name
    module_name = f"SHINEs.newmodel_{version}_100Hz"
...
meg_dir = '../../libribrain_test/data/lowpass100/'
...
predlist = []
print('model',len(modellist))

for model_path in modellist:
    model = load_model_and_checkpoint(model_path)
