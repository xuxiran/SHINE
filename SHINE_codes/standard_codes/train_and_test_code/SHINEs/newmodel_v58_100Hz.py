...
class Satt(nn.Module):
    def __init__(self, embed_size=64, heads=8):
...
class Extractor(nn.Module):
    def __init__(self, input_channels=64*2):
...
class OutputContext(nn.Module):
    def __init__(self, input_channels=64):
        ...
        self.conv = nn.Conv1d(input_channels, 64, 64)
...
class SHINE(nn.Module):
    def __init__(self, nb_blocks=6,emb = 64, output_dim=1):
        ...
        self.spatialattention2 = nn.Linear(306, 64)
        self.extractor = Extractor()
        self.satt = Satt()
        self.output_context = OutputContext()
        self.nb_blocks = nb_blocks
        self.linear_layer = nn.Linear(64*2, 64)
        self.fc = nn.Linear(64*5, 64)
        ...
        self.cnn = nn.Conv1d(64*3, 64, 101, padding=50)
    ...
    def forward(self, x):
        ...
        eeg_out = torch.cat((eeg_out, bm_out, WavNet_out), dim=2)
