...
class SHINE(nn.Module):
    def __init__(self, input_channels=306, out_channels=1, sub_num=12,
        ...
        self.init_conv = nn.Sequential(Conv(input_channels, sub_dim, kernel_size=1), nn.ReLU())
    ...
    def forward(self, input_data):
        ...
        x = x.transpose(-1,-2)
        # x = torch.bmm(x, self.subject_layer[sub_id])
        if hasattr(self, 'post_linear'):
...
if __name__ == '__main__':
    ...
    net =  SHINE(input_channels=306, out_channels=1)
