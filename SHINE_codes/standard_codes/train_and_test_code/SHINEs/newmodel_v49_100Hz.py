...
class MagicBlock(nn.Module):
    def __init__(self, k=1,channels=[in_channel,in_channel,in_channel,in_channel*2], kernel_size=25, dropout=0):
        super().__init__()
        self.k = k
        if kernel_size == 3:
...
class Extractor(nn.Module):
    def __init__(self, input_channels=in_channel*2):
        ...
        self.convs1 = nn.Conv1d(in_channel*4, in_channel, 1)
        self.convt1 = nn.Conv1d(in_channel*5, in_channel*5, 12,groups=in_channel*5)

        self.convs2 = nn.Conv1d(in_channel*5, in_channel, 1)
        self.convt2 = nn.Conv1d(in_channel*5, in_channel*5, 12,groups=in_channel*5)

        self.convs3 = nn.Conv1d(in_channel*5, in_channel, 1)
        self.convt3 = nn.Conv1d(in_channel*5, in_channel*5, 12,groups=in_channel*5)

        self.convs4 = nn.Conv1d(in_channel*5, in_channel, 1)
        self.convt4 = nn.Conv1d(in_channel*5, in_channel*5, 12,groups=in_channel*5)

        self.conv5 = nn.Conv1d(in_channel*5, in_channel*2, 12)

        self.norm1 = nn.LayerNorm(in_channel)
        self.norm2 = nn.LayerNorm(in_channel*2)
        self.norm3 = nn.LayerNorm(in_channel*5)
...
class SHINE(nn.Module):
    def __init__(self, nb_blocks=6,emb = in_channel, output_dim=1):
        ...
        self.fc = nn.Linear(in_channel*3, in_channel)
    ...
    def forward(self, x):
        ...
        eeg_out_me = torch.zeros_like(eeg)
        for i in range(self.nb_blocks):

            eeg_out = torch.cat((eeg,eeg_out,eeg_out_att,eeg_out_me), dim=2)
            ...
            eeg_out_me = self.ME(eeg_out)

            # eeg_out_att = eeg_out_att * eeg_out

        eeg_out = torch.cat((eeg_out,eeg_out_att,eeg_out_me), dim=2)
